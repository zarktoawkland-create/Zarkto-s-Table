...
function library_ensure_schema($conn) {
    $conn->query("CREATE TABLE IF NOT EXISTS coc_library_modules (
...
function library_module_row($row) {
    return [
        'id' => (int)$row['id'],
...
if ($action === 'list') {
    ...
    if ($section === 'restricted') {
        ...
        if ($query !== '') {
            $stmt = $conn->prepare("SELECT id, created_at, title, description, content, author_name, author_id, type, downloads, min_players, max_players
                FROM coc_library_modules
                WHERE (type = 'ai_adapt' OR (type = 'ai_adapt_private' AND author_id = ?)) AND title LIKE ?
                ORDER BY created_at DESC LIMIT {$listLimit}");
            $stmt->bind_param('ss', $userId, $like);
        } else {
            $stmt = $conn->prepare("SELECT id, created_at, title, description, content, author_name, author_id, type, downloads, min_players, max_players
                FROM coc_library_modules
                WHERE type = 'ai_adapt' OR (type = 'ai_adapt_private' AND author_id = ?)
                ORDER BY created_at DESC LIMIT {$listLimit}");
